...
on launchRS3()
	...
	--     forever. Re-applied each launch because a Wine upgrade's wineboot --update recreates the
	--     link. Full reasoning in drop_host_root_drive (lib/wine.sh). 2026-07-31, issue #32.
	set hygiene to "if ! pgrep -f 'AiMRS3-64' >/dev/null 2>&1; then " & ¬
		...
		"dd=" & quoted form of (root & "/prefix/dosdevices") & "; rm -f \"$dd/z:\" \"$dd/z::\" 2>/dev/null; " & ¬
