...
printf 'prev-log-marker\n' > "$ROOT/logs/run.log.1"   # previous generation (launch rotation)
...
assert_true "grep -q prev-log-marker '$OUT/run.log.1'" "rotated run.log.1 was copied"
assert_true "! grep -q 'run.log.1' '$OUT/README.txt'" "run.log.1 is optional (never listed as missing)"
